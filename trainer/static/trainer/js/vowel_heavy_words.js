...
(function () {
var vowelHeavyWords = vowelHeavyWords_temp;
vowelHeavyWords.word_matches_guessed = [];
...
$("#final_score_button").click(function() {
    ...
    buildFinalWordList();
    var total_possible_points = (vowelHeavyWords.total_word_count * 3);
    var all_non_guessed_words = [];
    for (var i=0; i<vowelHeavyWords.word_matches.length; i++) {
        var temp_word = vowelHeavyWords.word_matches[i];
        if (vowelHeavyWords.word_matches_guessed.indexOf(temp_word) == -1) {
            all_non_guessed_words.push(temp_word);
            }
        }
    vowelHeavyWords.total_point_count += (all_non_guessed_words.length * -1);
    var updated_text = ('<p class="lead">Your final score was <strong>' 
        + vowelHeavyWords.total_point_count 
        + '</strong>, out of a possible ' 
        + total_possible_points
        + ' points.</p><p>You guessed '
        + vowelHeavyWords.word_matches_guessed.length
        + ' out of '
        + vowelHeavyWords.total_word_count
    ...
    $("#word_letter_point_count").text(vowelHeavyWords.total_point_count);
    });
...
function checkAndUpdateUserWord(user_word) {
    ...
    if (vowelHeavyWords.word_matches.indexOf(user_word) != -1) {
        user_word_valid = true;
        if (vowelHeavyWords.word_matches_guessed.indexOf(user_word) == -1) {
            vowelHeavyWords.word_matches_guessed.push(user_word);
            vowelHeavyWords.word_matches_guessed.sort();
...
function updatePointCount(user_word_valid) {
    if (user_word_valid == true) {
        vowelHeavyWords.total_point_count += 3;
        }
    else {
        vowelHeavyWords.total_point_count -= 1;
        }
    
    $("#word_letter_point_count").html(vowelHeavyWords.total_point_count);
    }
    

function addWordMatchGuessed(user_word) {
    var word_match_letter_text = '';
    for (var i=0; i < vowelHeavyWords.word_matches_guessed.length; i++) {
        if (vowelHeavyWords.word_matches_guessed[i] == user_word) {
            word_match_letter_text += '<span class="word_match_correct">' + vowelHeavyWords.word_matches_guessed[i] + '</span>';
            }
        else {
            word_match_letter_text += vowelHeavyWords.word_matches_guessed[i];
            }
        
        if (i < vowelHeavyWords.word_matches_guessed.length - 1) {
...
function buildFinalWordList() {
    ...
    for (var i=0; i < vowelHeavyWords.word_matches.length; i++) {
        ...
        if (i < vowelHeavyWords.word_matches.length - 1) {
